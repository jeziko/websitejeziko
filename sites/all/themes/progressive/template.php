...
function progressive_username($variables) {
  if (isset($variables['link_path']) && 0) {
...
function progressive_field__field_social_icons($variables) {
  ...
  if(count($variables['items'])) {
    ...
    for ($i = 0; $i < count($variables['items']); $i++) {
      $output .= '<div class = "item"><a class="sbtnf sbtnf-rounded color color-hover ' . $variables['items'][$i]['#element']['title'] . '" href="' . $variables['items'][$i]['#element']['url'] . '"></a></div>';
    }
    $output .= '</div>';
  }
  return $output;
...
function progressive_field__field_old_price($variables) {
  ...
  if(count($variables['items'])) {
    for ($i = 0; $i < count($variables['items']); $i++) {
      $output .= '<span class="price-old">' . variable_get('uc_currency_sign', '$') . $variables['items'][$i]['#markup'] . '</span>';
    }
  }
  return $output;
}
