...
function menu() {
  ...
  if (body.width() <= 979 ) {
	...
	$(primary).find('ul:not(.sub)').removeAttr('style').find('li').removeClass('active');
  }
...
function initialize() {
  ...
  mapCanvas.each(function () {
	...
	var image = '/' + Drupal.settings.theme_path + '/img/svg/map-marker.svg';
...
(function() {
    ...
    attach: function (context, settings) {
      ...
      $('.cart-header .product-remove:not(.ajax-processed)').once('ajax').click(function() {
        ...
        $(this).parents('li').animate({'opacity': 0, 'height' : 0}, 700, function() {
          $(this).remove();
          $('.cart-header .cart-count').text(parseInt($('.cart-header .cart-count').text()) - 1);
        });
        return false;
      });
  ...
  Drupal.behaviors.removefromcompare = {
    attach: function (context, settings) {
      // Remove from block Cart
      $('#compare-table .product-remove:not(.ajax-processed)').once('ajax').click(function() {
        $.post($(this).attr('href'));
  ...
  $(document).ready(function() {
    ...
    $(document).bind('flagGlobalAfterLinkUpdate', function(event, data) {
      if(data.flagName == 'compare' || data.flagName == 'wishlist') {
        ...
        if(new_value > 0) {
          $('.flag-status-' + data.flagName).show();
        }
        else {
          $('.flag-status-' + data.flagName).hide();
        }
        $('.flag-count-' + data.flagName).text(new_value);
      }
    });

    if(typeof($.fn.slider) !== 'undefined' && $('#filter').length) {
      var from_date = parseInt($('#filter').parents('.view').find('.form-control[name*=min]').val().substr(0, 4));
      var to_date = parseInt($('#filter').parents('.view').find('.form-control[name*=max]').val().substr(0, 4));
      $('#filter').attr('value', from_date + ';' + to_date);
      $('#filter').slider({
        from: from_date - 3,
        to: to_date + 3,
        limits: false,
        ...
        callback: function(value) {
          ...
          $('#filter').parents('.view').find('.form-control[name*=min]').val(dates[0]);
          $('#filter').parents('.view').find('.form-control[name*=max]').val(dates[1]).change();
        }
      });
    }

    if(typeof($.fn.slider) !== 'undefined' && $('#price-filter').length) {
      var from_price = parseInt($('#price-filter').parents('.view').find('.form-item-sell-price-min .form-control').val());
      var to_price = parseInt($('#price-filter').parents('.view').find('.form-item-sell-price-max .form-control').val());
      $('#price-filter').attr('value', from_price + ';' + to_price);
      $('#price-filter').slider({
        ...
        callback: function(value) {
          ...
          $('#price-filter').parents('.view').find('.form-item-sell-price-min .form-control').val(prices[0]);
          $('#price-filter').parents('.view').find('.form-item-sell-price-max .form-control').val(prices[1]).change();
        }
      });
    }
    ...
    $('.add-cart.js-active').click(function() {
      $.post($(this).attr('href'), function() {
        window.location.replace(Drupal.settings.basePath + 'cart');
      })
      return false;
    });
